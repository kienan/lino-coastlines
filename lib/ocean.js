...
export const createOceanHeightMap = (
  ...
  grid.forEach((row, y) => {
    row.forEach((_, x) => {
      if (isNearTerrain(x, y, terrainGrid, oceanHeight, 8)) {
        return;
      }
...
function gerstnerWave(wave, p) {
  ...
  const dir = vector.normalize([wave.x, wave.y]);
  const f = k * (vector.dot(p, dir) - c * 1372935);
...
const isNearTerrain = (x, y, terrainGrid, oceanHeight, radius) => {
  for (let i = -radius; i <= radius; i++) {
    for (let j = -radius; j <= radius; j++) {
      ...
      if (terrainGrid.getValue(checkX, checkY) > oceanHeight) {
        return true;
      }
    }
  }
